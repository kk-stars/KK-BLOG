<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:76:"D:\PHPWAMP_IN3\wwwroot\kkstars/application/babysbreath\view\login\login.html";i:1548123843;}*/ ?>
<!doctype html>
<html>
<head>
	...
	<div class="content1">
		...
		<form action="" method="post">
			...
			<?php if($conf['code'] == '1'): ?>
				...
				<img src="<?php echo captcha_src(); ?>" id="" style="width:34%;height: 47px;border: 1px solid #fff;border-radius: 4px;margin-top: -5px;" onclick="this.src='<?php echo captcha_src(); ?>?d='+Math.random();" />
				<div class="button-row">
					<input type="button" onclick="login(<?php echo $conf['code']; ?>)" class="sign-in" value="登录" />
...
function login(code){
	...
	if(oNval == ''){
		...
		$.post('__URL__/login',{'loginName':oNval,'loginPassword':oPval,
			'checkCode':oCheckCode,'code':oCode},function(info){
			...
			if (i.code == 1){
			    ...
			    setTimeout('location.href = "<?php echo url('babysbreath/Index/index'); ?>";', 1000);
			}else{
				//登录失败
	            /*	 提示登录失败信息	*/
				$('#infoBody').html(i.message);
...
<div class="footer">
	<p>Copyright ©2019 <?php echo $conf['url']; ?> Powered By<a href="http://<?php echo $conf['url']; ?>" target="_blank" title=""><?php echo $conf['keywords']; ?></a> Version 5.2.6</p><a href="http://www.miitbeian.gov.cn/" target="_blank" title=""><?php echo $conf['beian']; ?></a>
</div>
