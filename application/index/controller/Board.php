        ...
        if(request()->isPost()){
            ...
            if($data){
                if($data['messageContent'] == ''){
                    $info = array('code' => '-1','message' => '请输入留言内容');
                    echo json_encode($info);die;
                }else{
                    $insert = db('message') -> insert($data);
                    $newMessage = db('message') -> where('status',1) -> order('addTime desc') -> find();
                    if($insert){
                        $info = array('code' => '1','message' => '留言成功','data' => $newMessage);

                        //操作记录
                        $op = new Operation();
                        $op -> op('add','留言','访问者：'.$data['randAtorName'],'留言ID：'.$newMessage['messageId']);
                    }else{
            ...
            if($reply){
                if($reply['replyContent'] == ''){
                    ...
                    if($insResult){
                        ...
                        $op -> op('replyMessage','回复留言',$op_admin,'回复留言ID：'.$showReply['replyId']);
                    }else{
