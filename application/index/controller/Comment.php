...
class Comment extends Comm{
    ...
    public function add(){
        if(request()->isPost()){
            $replace = preg_replace( '/[^0-9a-fA-F:., ]/', '',$_SERVER['REMOTE_ADDR'] );
            dump($replace);die;
            ...
            if($data !== null){
                ...
                $data['commentAtor'] = $getAtorName -> getName(2);

                db('comment') -> insert($data);
                $cid = db('comment') -> where('commentAtor',$data['commentAtor']) -> field('commentId,addTime') -> order('addTime desc') -> limit(1) -> find();
                $cid['addTime'] = date('Y年m月d日 H:i:s',strtotime($cid['addTime']));
                $info = array('code' => 1,'message' => '评论成功','data' => $data,'cid' => $cid);

                //操作记录
                $op = new Operation();
                $op_ip = $_SERVER['REMOTE_ADDR'];
                $op -> op('add','评论','访问者',$cid['commentId'],'');
            }else{
